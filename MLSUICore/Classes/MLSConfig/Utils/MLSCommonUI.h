...
#define CodeAttributes(_fontSize) @{NSFontAttributeName: CodeFontMake(_fontSize), NSForegroundColorAttributeName: [MLSThemeManager sharedInstance].currentTheme.themeCodeColor}

/// QMUIButton 系列 Demo 里的一行高度
extern const CGFloat QMUIButtonSpacingHeight;

@interface MLSCommonUI : NSObject

+ (void)renderGlobalAppearances;
@end

@interface MLSCommonUI (ThemeColor)

+ (UIColor *)randomThemeColor;
@end

@interface MLSCommonUI (Layer)
