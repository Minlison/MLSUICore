...
#pragma mark - Global Color
    ...
    QMUICMI.maskDarkColor = UIColorMakeWithRGBA(0, 0, 0, .7f);                 // UIColorMask : 深色的背景遮罩，默认用于 QMAlertController、QMUIDialogViewController 等弹出控件的遮罩
    QMUICMI.maskLightColor = UIColorMakeWithRGBA(255, 255, 255, .5f);           // UIColorMaskWhite : 浅色的背景遮罩，QMUIKit 里默认没用到，只是占个位
    QMUICMI.separatorColor = [UIColor colorWithWhite:0 alpha:0.1];                        // UIColorSeparator : 全局默认的分割线颜色，默认用于列表分隔线颜色、UIView (QMUI_Border) 分隔线颜色
    QMUICMI.separatorDashedColor = [UIColor colorWithWhite:0 alpha:0.1];                     // UIColorSeparatorDashed : 全局默认的虚线分隔线的颜色，默认 QMUIKit 暂时没用到
...
#pragma mark - NavigationBar
    ...
    QMUICMI.navBarButtonFont = MLSSystemFont(17);                                  // NavBarButtonFont : QMUINavigationButtonTypeNormal 的字体（由于系统存在一些 bug，这个属性默认不对 UIBarButtonItem 生效）
    QMUICMI.navBarButtonFontBold = MLSBoldSystemFont(17);                          // NavBarButtonFontBold : QMUINavigationButtonTypeBold 的字体
    QMUICMI.navBarBackgroundImage = UIImageMake(@"navigationbar_background");   // NavBarBackgroundImage : UINavigationBar 的背景图
    QMUICMI.navBarShadowImage = [UIImage qmui_imageWithColor:[UIColor colorWithWhite:1 alpha:0.1] size:CGSizeMake([UIScreen mainScreen].bounds.size.width, 1) cornerRadius:0];                                  // NavBarShadowImage : UINavigationBar.shadowImage，也即导航栏底部那条分隔线
    QMUICMI.navBarBarTintColor = nil;                                           // NavBarBarTintColor : UINavigationBar.barTintColor，也即背景色
    QMUICMI.navBarTintColor = UIColorMakeWithHex(@"#000000");                                     // NavBarTintColor : QMUINavigationBar 的 tintColor，也即导航栏上面的按钮颜色
    QMUICMI.navBarTitleColor = UIColorMakeWithHex(@"#333333");                                 // NavBarTitleColor : UINavigationBar 的标题颜色，以及 QMUINavigationTitleView 的默认文字颜色
    QMUICMI.navBarTitleFont = MLSBoldSystemFont(18);                               // NavBarTitleFont : UINavigationBar 的标题字体，以及 QMUINavigationTitleView 的默认字体
    ...
    QMUICMI.navBarBackIndicatorImage = [UIImage qmui_imageWithShape:QMUIImageShapeNavBack size:CGSizeMake(9, 15) tintColor:[UIColor blackColor]];                                     // NavBarBackIndicatorImage : 导航栏的返回按钮的图片，图片尺寸建议为(13, 21)，否则最终的图片位置无法与系统原生的位置保持一致
    QMUICMI.navBarCloseButtonImage = [UIImage qmui_imageWithShape:QMUIImageShapeNavClose size:CGSizeMake(16, 16) tintColor:[UIColor blackColor]];     // NavBarCloseButtonImage : QMUINavigationButton 用到的 × 的按钮图片
    
    QMUICMI.navBarLoadingMarginRight = 3;                                       // NavBarLoadingMarginRight : QMUINavigationTitleView 里左边 loading 的右边距
    QMUICMI.navBarAccessoryViewMarginLeft = 8;                                  // NavBarAccessoryViewMarginLeft : QMUINavigationTitleView 里右边 accessoryView 的左边距
    ...
    QMUICMI.navBarAccessoryViewTypeDisclosureIndicatorImage = [[UIImage qmui_imageWithShape:QMUIImageShapeTriangle size:CGSizeMake(11, 8) tintColor:UIColorBlack] qmui_imageWithOrientation:(UIImageOrientationUp)];     // NavBarAccessoryViewTypeDisclosureIndicatorImage : QMUINavigationTitleView 右边箭头的图片
...
#pragma mark - Toolbar
    ...
    QMUICMI.toolBarButtonFont = MLSSystemFont(17);                                 // ToolBarButtonFont : QMUIToolbarButton 的字体
...
#pragma mark - TableView / TableViewCell
    ...
    QMUICMI.tableViewSectionHeaderFont = MLSBoldSystemFont(12);                                            // TableViewSectionHeaderFont : Plain 类型的 QMUITableView sectionHeader 里的文字字体
    QMUICMI.tableViewSectionFooterFont = MLSBoldSystemFont(12);                                            // TableViewSectionFooterFont : Plain 类型的 QMUITableView sectionFooter 里的文字字体
    ...
    QMUICMI.tableViewGroupedSectionHeaderFont = MLSSystemFont(12);                                         // TableViewGroupedSectionHeaderFont : Grouped 类型的 QMUITableView sectionHeader 里的文字字体
    QMUICMI.tableViewGroupedSectionFooterFont = MLSSystemFont(12);                                         // TableViewGroupedSectionFooterFont : Grouped 类型的 QMUITableView sectionFooter 里的文字字体
...
- (BOOL)shouldApplyTemplateAutomatically {
    BOOL result = ![MLSThemeManager sharedInstance].currentTheme || [[[NSUserDefaults standardUserDefaults] stringForKey:MLSSelectedThemeClassName] isEqualToString:NSStringFromClass(self.class)];
    if (result) {
        [MLSThemeManager sharedInstance].currentTheme = self;
    }
    return result;
}
...
- (UIColor *)themeTintColor {
    return UIColorMakeWithHex(@"#356ED7");
}
