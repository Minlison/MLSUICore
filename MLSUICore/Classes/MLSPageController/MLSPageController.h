...
@class MLSPageScrollView;
@class MLSPageController;

/*
 *  MLSPageController 的缓存设置，默认缓存为无限制，当收到 memoryWarning 时，会自动切换到低缓存模式 (MLSPageControllerCachePolicyLowMemory)，并在一段时间后切换到 High .
    收到多次警告后，会停留在到 MLSPageControllerCachePolicyLowMemory 不再增长
 *
 ...
 */
typedef NS_ENUM(NSInteger, MLSPageControllerCachePolicy) {
    MLSPageControllerCachePolicyDisabled   = -1,  // Disable Cache
    MLSPageControllerCachePolicyNoLimit    = 0,   // No limit
    MLSPageControllerCachePolicyLowMemory  = 1,   // Low Memory but may block when scroll
    MLSPageControllerCachePolicyBalanced   = 3,   // Balanced ↑ and ↓
    MLSPageControllerCachePolicyHigh       = 5    // High
};

typedef NS_ENUM(NSUInteger, MLSPageControllerPreloadPolicy) {
    MLSPageControllerPreloadPolicyNever     = 0, // Never pre-load controller.
    MLSPageControllerPreloadPolicyNeighbour = 1, // Pre-load the controller next to the current.
    MLSPageControllerPreloadPolicyNear      = 2  // Pre-load 2 controllers near the current.
};

typedef NS_ENUM(int, MLSPageControllerScrollDirection) {
	MLSPageControllerScrollDirectionHorizontal,
	MLSPageControllerScrollDirectionVertical,
};

NS_ASSUME_NONNULL_BEGIN
extern NSString *const MLSControllerDidAddToSuperViewNotification;
extern NSString *const MLSControllerDidFullyDisplayedNotification;
@protocol MLSPageControllerDataSource <NSObject>
@optional

/**
 *  To inform how many child controllers will in `MLSPageController`.
 *
 *  @param pageController The parent controller.
 *
 *  @return The value of child controllers's count.
 */
- (NSInteger)numbersOfChildControllersInPageController:(MLSPageController *)pageController;
 ...
 */
- (__kindof UIViewController *)pageController:(MLSPageController *)pageController viewControllerAtIndex:(NSInteger)index;

/**
 *  Each title you wanna show in the `MLSMenuView`
 *
 *  @param pageController The parent controller.
 *  @param index          The index of title.
 *
 *  @return A `NSString` value to show at the top of `MLSPageController`.
 */
- (NSString *)pageController:(MLSPageController *)pageController titleAtIndex:(NSInteger)index;
 ...
 */
- (CGRect)pageController:(MLSPageController *)pageController preferredFrameForContentView:(MLSPageScrollView *)contentView;
 ...
 */
- (CGRect)pageController:(MLSPageController *)pageController preferredFrameForMenuView:(MLSMenuView *)menuView;

@end

@protocol MLSPageControllerDelegate <NSObject>
@optional
 ...
 *
 *  @param pageController The parent controller (MLSPageController)
 ...
 */
- (void)pageController:(MLSPageController *)pageController lazyLoadViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageController:(MLSPageController *)pageController willCachedViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageController:(MLSPageController *)pageController willEnterViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
 ...
 *  @param info           A dictionary that includes some infos, such as: `index` / `title`
 */
- (void)pageController:(MLSPageController *)pageController didEnterViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;

- (BOOL)pageController:(MLSPageController *)pageController shouldEnterViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;

@end
@interface MLSPageController : MLSBaseViewController <MLSMenuViewDelegate, MLSMenuViewDataSource, UIScrollViewDelegate, MLSPageControllerDataSource, MLSPageControllerDelegate>
@property (nonatomic, weak) id<MLSPageControllerDelegate> delegate;
@property (nonatomic, weak) id<MLSPageControllerDataSource> dataSource;
...
/// 分割线
@property (nonatomic, assign) UIEdgeInsets menuSeparateInset;
@property (nonatomic, strong) UIColor *menuSeparateColor;
@property (nonatomic, assign) CGFloat menuSeparateWidth;
@property (nonatomic, assign) CGFloat menuSeparateHeight;
/// 分割线
/// 底部线条
@property (nonatomic, strong) UIColor *menuBottomLineColor;
@property (nonatomic, assign) CGFloat menuBottomLineWidth;
/// 底部线条
/**
 *  Menu view 的样式，默认为无下划线
 *  Menu view's style, now has two different styles, 'Line','default'
 */
@property (nonatomic, assign) MLSMenuViewStyle menuViewStyle;

@property (nonatomic, assign) MLSMenuViewLayoutMode menuViewLayoutMode;

/**
 方位
 */
@property (assign, nonatomic) MLSPageControllerScrollDirection direction;
/**
...
@property (nonatomic, nullable, strong) UIColor *progressBgColor;
 ...
 *  See `MLSPageConst.h` for more information.
 */
...
@property (nonatomic, assign) MLSPageControllerCachePolicy cachePolicy;

/** 预加载机制，在停止滑动的时候预加载 n 页 */
@property (nonatomic, assign) MLSPageControllerPreloadPolicy preloadPolicy;
...
/** 顶部导航栏 */
@property (nonatomic, nullable, weak) MLSMenuView *menuView;

/** 内部容器 */
@property (nonatomic, nullable, weak) MLSPageScrollView *scrollView;
...
/**
 Layout all views in MLSPageController
...
/// 快速创建全局默认配置的控制器
+ (instancetype)defaultPageControllerWithTitles:(nullable NSArray <NSString *>*)titles;
@end


@interface NSDictionary (MLSPageController)
