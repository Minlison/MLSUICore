...
@interface MLSMenuView () 
@property (nonatomic, weak) MLSMenuItem *selItem;
@property (nonatomic, strong) NSMutableArray *frames;
@property (nonatomic, readonly) NSInteger titlesCount;
@property (nonatomic, assign) NSInteger selectIndex;
@end

static NSInteger const MLSMenuItemTagOffset  = 6250;
static NSInteger const MLSBadgeViewTagOffset = 1212;

@implementation MLSMenuView

#pragma mark - Setter

- (void)setLayoutMode:(MLSMenuViewLayoutMode)layoutMode {
...
- (void)setSpeedFactor:(CGFloat)speedFactor {
    ...
    [self.scrollView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[MLSMenuItem class]]) {
            ((MLSMenuItem *)obj).speedFactor = self->_speedFactor;
        }
    }];
...
- (UIColor *)lineColor {
    if (!_lineColor) {
        _lineColor = [self colorForState:MLSMenuItemStateSelected atIndex:0];
    }
    return _lineColor;
}
...
- (UIColor *)colorForState:(MLSMenuItemState)state atIndex:(NSInteger)index {
...
- (CGFloat)sizeForState:(MLSMenuItemState)state atIndex:(NSInteger)index {
...
- (UIView *)badgeViewAtIndex:(NSInteger)index {
    ...
    badgeView.tag = index + MLSBadgeViewTagOffset;
...
- (MLSMenuItem *)itemAtIndex:(NSInteger)index {
    return (MLSMenuItem *)[self viewWithTag:(index + MLSMenuItemTagOffset)];
}
...
- (void)slideMenuAtProgress:(CGFloat)progress {
    ...
    NSInteger tag = (NSInteger)progress + MLSMenuItemTagOffset;
    CGFloat rate = progress - tag + MLSMenuItemTagOffset;
    MLSMenuItem *currentItem = (MLSMenuItem *)[self viewWithTag:tag];
    MLSMenuItem *nextItem = (MLSMenuItem *)[self viewWithTag:tag+1];
    if (rate == 0.0) {
...
- (void)selectItemAtIndex:(NSInteger)index {
    NSInteger tag = index + MLSMenuItemTagOffset;
    NSInteger currentIndex = self.selItem.tag - MLSMenuItemTagOffset;
    self.selectIndex = index;
    if (index == currentIndex || !self.selItem) { return; }
    
    MLSMenuItem *item = (MLSMenuItem *)[self viewWithTag:tag];
    [self.selItem setSelected:NO withAnimation:NO];
    self.selItem = item;
    [self.selItem setSelected:YES withAnimation:NO];
...
- (void)updateTitle:(NSString *)title atIndex:(NSInteger)index andWidth:(BOOL)update {
    ...
    MLSMenuItem *item = (MLSMenuItem *)[self viewWithTag:(MLSMenuItemTagOffset + index)];
    item.text = title;
    if (!update) { return; }
    [self resetFrames];
}
...
- (void)updateBadgeViewAtIndex:(NSInteger)index {
    UIView *oldBadgeView = [self.scrollView viewWithTag:MLSBadgeViewTagOffset + index];
    if (oldBadgeView) {
...
- (void)resetFramesFromIndex:(NSInteger)index {
    ...
    if (self.style == MLSMenuViewStyleDefault) { return; }
    if (self.style == MLSMenuViewStyleLine || self.style == MLSMenuViewStyleTriangle) {
...
- (void)resetBadgeFrame:(NSInteger)index {
    CGRect frame = [self.frames[index] CGRectValue];
    UIView *badgeView = [self.scrollView viewWithTag:(MLSBadgeViewTagOffset + index)];
    if (badgeView) {
...
- (void)makeStyle {
    CGRect frame = CGRectZero;
    if (self.style == MLSMenuViewStyleDefault) { return; }
    if (self.style == MLSMenuViewStyleLine) {
        self.progressHeight = self.progressHeight > 0 ? self.progressHeight : 2.0;
        frame = CGRectMake(0, self.frame.size.height - self.progressHeight - self.progressViewBottomSpace, self.scrollView.contentSize.width, self.progressHeight);
    } else {
        self.progressHeight = self.progressHeight > 0 ? self.progressHeight : self.frame.size.height * 0.8;
    ...
    [self addProgressViewWithFrame:frame
                        isTriangle:(self.style == MLSMenuViewStyleTriangle)
                         hasBorder:(self.style == MLSMenuViewStyleSegmented)
                            hollow:(self.style == MLSMenuViewStyleFloodHollow)
                      cornerRadius:self.progressViewCornerRadius];
}

- (void)deselectedItemsIfNeeded {
    [self.scrollView.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (![obj isKindOfClass:[MLSMenuItem class]] || obj == self.selItem) { return; }
        [(MLSMenuItem *)obj setSelected:NO withAnimation:NO];
    }];
}
...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        MLSMenuItem *item = [[MLSMenuItem alloc] initWithFrame:frame];
        item.tag = (i + MLSMenuItemTagOffset);
        item.delegate = self;
        item.text = [self.dataSource menuView:self titleAtIndex:i];
        item.textAlignment = NSTextAlignmentCenter;
        item.userInteractionEnabled = YES;
        item.backgroundColor = [UIColor clearColor];
        item.normalSize    = [self sizeForState:MLSMenuItemStateNormal atIndex:i];
        item.selectedSize  = [self sizeForState:MLSMenuItemStateSelected atIndex:i];
        item.normalColor   = [self colorForState:MLSMenuItemStateNormal atIndex:i];
        item.selectedColor = [self colorForState:MLSMenuItemStateSelected atIndex:i];
        ...
        if (i != self.titlesCount - 1) {
            CGRect frameSec = [self.frames[i + 1] CGRectValue];
            /// add separate
            CGFloat x =  CGRectGetMaxX(frame) + (CGRectGetMinX(frameSec) - CGRectGetMaxX(frame) - self.separateWidth) * 0.5;
            CGFloat height = CGRectGetMaxY(frame) - self.separateInset.top - self.separateInset.bottom;
            if (self.separateHeight > 0.1) {
                height = self.separateHeight;
            }
            CGFloat y = (CGRectGetMaxY(frame) - height ) * 0.5;
            UIView *separateView = [[UIView alloc] init];
            separateView.frame = CGRectMake(x, y, self.separateWidth, height);
...
- (void)addProgressViewWithFrame:(CGRect)frame isTriangle:(BOOL)isTriangle hasBorder:(BOOL)hasBorder hollow:(BOOL)isHollow cornerRadius:(CGFloat)cornerRadius {
    MLSProgressView *pView = [[MLSProgressView alloc] initWithFrame:frame];
    ...
    pView.backgroundColor = self.progressBgColor ?:[UIColor clearColor];
    self.progressView = pView;
    pView.progress = (float)self.selectIndex / (float)self.titlesCount;
...
- (void)didPressedMenuItem:(MLSMenuItem *)menuItem {
    ...
    if ([self.delegate respondsToSelector:@selector(menuView:shouldSelesctedIndex:)]) {
        BOOL should = [self.delegate menuView:self shouldSelesctedIndex:menuItem.tag - MLSMenuItemTagOffset];
        if (!should) {
            return;
        }
    }
    
    CGFloat progress = menuItem.tag - MLSMenuItemTagOffset;
    [self.progressView moveToPostion:progress];
    
    NSInteger currentIndex = self.selItem.tag - MLSMenuItemTagOffset;
    _selectIndex = menuItem.tag - MLSMenuItemTagOffset;
    if ([self.delegate respondsToSelector:@selector(menuView:didSelesctedIndex:currentIndex:)]) {
        [self.delegate menuView:self didSelesctedIndex:_selectIndex currentIndex:currentIndex];
    }
    ...
    NSTimeInterval delay = self.style == MLSMenuViewStyleDefault ? 0 : 0.3f;
