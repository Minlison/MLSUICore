...
@interface MLSSegmentedScrollView : UIScrollView
@end

@interface MLSSegmentedControl ()
...
@property (nonatomic, strong) MLSSegmentedScrollView *scrollView;

@end

@implementation MLSSegmentedScrollView
...
@implementation MLSSegmentedControl
...
- (id)initWithSectionTitles:(NSArray *)sectiontitles {
	...
	if (self) {
		...
		self.type = MLSSegmentedControlTypeText;
	}
...
- (id)initWithSectionImages:(NSArray*)sectionImages sectionSelectedImages:(NSArray*)sectionSelectedImages {
	...
	if (self) {
		...
		self.type = MLSSegmentedControlTypeImages;
	}
...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles {
	...
	if (self) {
		...
		self.sectionSelectedImages = sectionSelectedImages;
		self.sectionTitles = sectiontitles;
		self.type = MLSSegmentedControlTypeTextImages;
	}
...
- (void)commonInit {
	self.scrollView = [[MLSSegmentedScrollView alloc] init];
	...
	self.selectionStyle = MLSSegmentedControlSelectionStyleTextWidthStripe;
	self.selectionIndicatorLocation = MLSSegmentedControlSelectionIndicatorLocationUp;
	self.segmentWidthStyle = MLSSegmentedControlSegmentWidthStyleFixed;
...
- (void)setSelectionIndicatorLocation:(MLSSegmentedControlSelectionIndicatorLocation)selectionIndicatorLocation {
	_selectionIndicatorLocation = selectionIndicatorLocation;
	
	if (selectionIndicatorLocation == MLSSegmentedControlSelectionIndicatorLocationNone) {
...
- (void)setSegmentWidthStyle:(MLSSegmentedControlSegmentWidthStyle)segmentWidthStyle {
	// Force MLSSegmentedControlSegmentWidthStyleFixed when type is MLSSegmentedControlTypeImages.
	if (self.type == MLSSegmentedControlTypeImages) {
		_segmentWidthStyle = MLSSegmentedControlSegmentWidthStyleFixed;
	} else {
		_segmentWidthStyle = segmentWidthStyle;
	}
}

- (void)setBorderType:(MLSSegmentedControlBorderType)borderType {
...
- (void)drawRect:(CGRect)rect {
	...
	if (self.type == MLSSegmentedControlTypeText) {
		[self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
			...
			BOOL locationUp = (self.selectionIndicatorLocation == MLSSegmentedControlSelectionIndicatorLocationUp);
			BOOL selectionStyleNotBox = (self.selectionStyle != MLSSegmentedControlSelectionStyleBox);
			...
			CGRect rect;
			if (self.segmentWidthStyle == MLSSegmentedControlSegmentWidthStyleFixed) {
	...
	} else if (self.type == MLSSegmentedControlTypeImages) {
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
			UIImage *icon = iconImage;
			CGFloat imageWidth = icon.size.width;
			CGFloat imageHeight = icon.size.height;
			CGFloat y = roundf(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == MLSSegmentedControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
	...
	} else if (self.type == MLSSegmentedControlTypeTextImages){
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
			UIImage *icon = iconImage;
			CGFloat imageWidth = icon.size.width;
			CGFloat imageHeight = icon.size.height;
			...
			} else if (self.segmentWidthStyle == MLSSegmentedControlSegmentWidthStyleDynamic) {
				// When we are drawing dynamic widths, we need to loop the widths array to calculate the xOffset
				CGFloat xOffset = 0;
				NSInteger i = 0;
	...
	if (self.selectedSegmentIndex != MLSSegmentedControlNoSegment) {
		if (self.selectionStyle == MLSSegmentedControlSelectionStyleArrow) {
			...
			if (!self.selectionIndicatorStripLayer.superlayer) {
				...
				if (self.selectionStyle == MLSSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
	...
	// Border layer
	if (self.borderType & MLSSegmentedControlBorderTypeTop) {
...
- (void)setArrowFrame {
	...
	if (self.selectionIndicatorLocation == MLSSegmentedControlSelectionIndicatorLocationDown) {
	...
	if (self.selectionIndicatorLocation == MLSSegmentedControlSelectionIndicatorLocationUp) {
...
- (CGRect)frameForSelectionIndicator {
	...
	if (self.selectionStyle == MLSSegmentedControlSelectionStyleArrow) {
		...
		if (self.selectionStyle == MLSSegmentedControlSelectionStyleTextWidthStripe &&
		    sectionWidth <= self.segmentWidth &&
		    self.segmentWidthStyle != MLSSegmentedControlSegmentWidthStyleDynamic) {
			CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
			CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
			...
			if (self.segmentWidthStyle == MLSSegmentedControlSegmentWidthStyleDynamic) {
...
- (void)updateSegmentsRects {
	...
	if (self.type == MLSSegmentedControlTypeText && self.segmentWidthStyle == MLSSegmentedControlSegmentWidthStyleFixed) {
		[self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
			CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
			self.segmentWidth = MAX(stringWidth, self.segmentWidth);
		}];
	} else if (self.type == MLSSegmentedControlTypeText && self.segmentWidthStyle == MLSSegmentedControlSegmentWidthStyleDynamic) {
	...
	} else if (self.type == MLSSegmentedControlTypeTextImages && self.segmentWidthStyle == MLSSegmentedControlSegmentWidthStyleFixed){
		//lets just use the title.. we will assume it is wider then images...
		[self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
			CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
			self.segmentWidth = MAX(stringWidth, self.segmentWidth);
		}];
	} else if (self.type == MLSSegmentedControlTypeTextImages && MLSSegmentedControlSegmentWidthStyleDynamic) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if (CGRectContainsPoint(enlargeRect, touchLocation)) {
		...
		} else if (self.type == MLSSegmentedControlTypeTextImages || self.type == MLSSegmentedControlTypeText) {
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
	...
	if (index == MLSSegmentedControlNoSegment) {
		...
		if (animated) {
			...
			if(self.selectionStyle == MLSSegmentedControlSelectionStyleArrow) {
				...
				if ([self.selectionIndicatorStripLayer superlayer] == nil) {
					...
					if (self.selectionStyle == MLSSegmentedControlSelectionStyleBox && [self.selectionIndicatorBoxLayer superlayer] == nil)
						[self.scrollView.layer insertSublayer:self.selectionIndicatorBoxLayer atIndex:0];
