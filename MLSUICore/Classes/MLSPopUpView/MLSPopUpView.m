...
@implementation CALayer (MLSAnimationAdditions)
...
@interface MLSPopUpView () <CAAnimationDelegate>
@end
#endif


@implementation MLSPopUpView
{
    BOOL _shouldAnimate;
...
- (void)setFrame:(CGRect)frame arrowOffset:(CGFloat)arrowOffset colorOffset:(CGFloat)colorOffset text:(NSString *)text
{
    ...
    if (self.position == MLSProgressPopUpPositionBottom) {
        archorY = -(frame.origin.y / CGRectGetHeight(frame));
    }
    self.layer.anchorPoint = CGPointMake(anchorX, archorY);
    self.layer.position = CGPointMake(CGRectGetMinX(frame) + CGRectGetWidth(frame)*anchorX, -self.popOffset.horizontal);
    if (self.position == MLSProgressPopUpPositionBottom) {
            self.layer.position = CGPointMake(CGRectGetMinX(frame) + CGRectGetWidth(frame)*anchorX, self.popOffset.horizontal);
    }
...
- (UIBezierPath *)pathForRect:(CGRect)rect withArrowOffset:(CGFloat)arrowOffset {
    if (self.position == MLSProgressPopUpPositionTop) {
        return [self topPathForRect:rect withArrowOffset:arrowOffset];
    }
    return [self bottomPathForRect:rect withArrowOffset:arrowOffset];
}
- (UIBezierPath *)topPathForRect:(CGRect)rect withArrowOffset:(CGFloat)arrowOffset;
{
...
- (UIBezierPath *)bottomPathForRect:(CGRect)rect withArrowOffset:(CGFloat)arrowOffset;
{
    if (CGRectEqualToRect(rect, CGRectZero)) return nil;
    ...
    CGRect roundedRect = rect;
    roundedRect.size.height -= _arrowLength;
    roundedRect.origin.y = _arrowLength;
    UIBezierPath *popUpPath = [UIBezierPath bezierPathWithRoundedRect:roundedRect cornerRadius:_cornerRadius];
    
    // Create arrow path
    CGFloat maxX = CGRectGetMaxX(roundedRect); // prevent arrow from extending beyond this point
    CGFloat arrowTipX = CGRectGetMidX(rect) + arrowOffset;
    CGPoint tip = CGPointMake(MAX(arrowTipX, 0), 0);
    
    CGFloat arrowLength = CGRectGetHeight(roundedRect)/2.0 + _arrowLength;
//    CGFloat x = arrowLength * tan(45.0 * M_PI/180); // x = half the length of the base of the arrow
    CGFloat x = (CGRectGetHeight(rect) - arrowLength) * tan(45.0 * M_PI/180); // x = half the length of the base of the arrow
    
    UIBezierPath *arrowPath = [UIBezierPath bezierPath];
    
    [arrowPath moveToPoint:tip];
    [arrowPath addLineToPoint:CGPointMake(MAX(arrowTipX - x, 0),arrowLength)];
    [arrowPath addLineToPoint:CGPointMake(MIN(arrowTipX + x, maxX), arrowLength)];
    [arrowPath addLineToPoint:tip];
    [arrowPath addLineToPoint:tip];
    [arrowPath closePath];
    [popUpPath appendPath:[arrowPath bezierPathByReversingPath]];
...
- (void)layoutSubviews
{
    ...
    if (self.position == MLSProgressPopUpPositionBottom) {
        textRect = CGRectMake(self.bounds.origin.x,
                              (self.bounds.size.height-_arrowLength-textHeight)/2 + _arrowLength,
                              self.bounds.size.width, textHeight);
    }
