...
@interface MLSProgressPopUpView() <MLSPopUpViewDelegate>
@property (strong, nonatomic) NSNumberFormatter *numberFormatter;
@property (strong, nonatomic) MLSPopUpView *popUpView;
@end

@implementation MLSProgressPopUpView
{
...
- (void)setPosition:(MLSProgressPopUpPosition)position {
    _position = position;
    self.popUpView.position = position;
}

- (void)setTextEdgeInsets:(UIEdgeInsets)textEdgeInsets {
    _textEdgeInsets = textEdgeInsets;
    self.popUpView.textEdgeInsets = textEdgeInsets;
}
...
- (void)setDataSource:(id<MLSProgressPopUpViewDataSource>)dataSource
{
...
- (void)setup
{
    ...
    _progressLayer.cornerRadius = self.progressCornerRadius;
    ...
    self.popUpView = [[MLSPopUpView alloc] initWithFrame:CGRectZero];
    self.popUpViewColor = [UIColor colorWithHue:0.6 saturation:0.6 brightness:0.5 alpha:0.8];
    self.popUpView.position = self.position;
    self.popUpView.alpha = 0.0;
    self.popUpView.popOffset = self.popOffset;
    self.popUpView.textEdgeInsets = self.textEdgeInsets;
...
- (void)updatePopUpView
{
    ...
    popUpViewSize.width += self.textEdgeInsets.left + self.textEdgeInsets.right;
    popUpViewSize.height += self.textEdgeInsets.top + self.textEdgeInsets.bottom;
    ...
    CGFloat xPos = 0;
    if (self.revert) {
        xPos = (CGRectGetWidth(bounds) * (1-self.progress)) - popUpViewSize.width/2;
    } else {
        xPos = (CGRectGetWidth(bounds) * self.progress) - popUpViewSize.width/2;
    }
    CGRect popUpRect = CGRectMake(xPos, CGRectGetMinY(bounds)-popUpViewSize.height,
                                  popUpViewSize.width, popUpViewSize.height);
    if (self.position == MLSProgressPopUpPositionBottom) {
        popUpRect = CGRectMake(xPos, CGRectGetMaxY(bounds),
                                      popUpViewSize.width, popUpViewSize.height);
...
- (void)updateProgressLayer
{
    ...
    if (self.revert) {
        _progressLayer.anchorPoint = CGPointMake(1, 1);
        CGFloat progressWidth = CGRectGetWidth(self.bounds) * self.progress;
        _progressLayer.frame = CGRectMake(CGRectGetWidth(self.bounds) - progressWidth, 0, progressWidth, self.bounds.size.height);
    } else {
        _progressLayer.anchorPoint = CGPointMake(0, 0);
        _progressLayer.frame = CGRectMake(0, 0, CGRectGetWidth(self.bounds)  * self.progress, self.bounds.size.height);
    }
}
